...
void mex_g_fun(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+3, casadi_s1, w+9);
  if (--argc>=0) arg[2] = casadi_from_mex(argv[2], w+5, casadi_s2, w+9);
  if (--argc>=0) arg[3] = casadi_from_mex(argv[3], w+5, casadi_s0, w+9);
  if (--argc>=0) arg[4] = casadi_from_mex(argv[4], w+8, casadi_s2, w+9);
  --resc;
  res[0] = w+8;
  g_fun_incref();
  mem = g_fun_checkout();
  i = g_fun(arg, res, iw, w+9, mem);
